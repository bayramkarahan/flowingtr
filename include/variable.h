    ...
    bool evaluate(const QList<VariableRecord>& vars) {
        ...
        if (valueType == "string") {
            ...
            case 2: { // İki değişkenli işlem
                QRegularExpression rx(R"((\w+)\s*([\+\-\*/])\s*(\w+))");
                auto match = rx.match(rightExpr);
                if (!match.hasMatch()) return false;

                QString var1 = match.captured(1);
                QString op   = match.captured(2);
                QString var2 = match.captured(3);
                ...
                QString result = "";
                if (op == "+") {
                    result = varVal1 + varVal2; // string birleştirme
                } else return false;
        ...
        if (valueType == "float") {
            ...
            switch (operationType) {
            case 0: { // Sabit atama
                float val = rightExpr.toFloat(&ok);
                //if (!ok) return false;
                value = QString::number(val);
                return true;
            }
            ...
            case 2: { // İki değişkenli işlem
                ...
                for (const auto& v : vars) {
                    if (v.label == var1.trimmed()) {
                        val1 = v.value.toFloat(&ok);
                        found1 = ok;
                    }
                    if (v.label == var2.trimmed()) {
                        val2 = v.value.toFloat(&ok);
                        found2 = ok;
                    }
                }
               /// if (!found1 || !found2) return false;
            ...
            case 3: { // Değişken + sabit sayı
                ...
                float val2 = numStr.trimmed().toFloat(&ok);
               // if (!ok) return false;
                float result = 0;
                if (op.trimmed() == "+") result = val1 + val2;
                else if (op.trimmed() == "-") result = val1 - val2;
            ...
            case 4: { // sabit sayı + sabit sayı
                ...
                float val1 = numStr1.trimmed().toFloat(&ok);
                //if (!ok) return false;

                float val2 = numStr2.trimmed().toFloat(&ok);
                //if (!ok) return false;
            ...
            case 5: { // Sabit atama
                float val = rightExpr.split(" ")[3].toFloat(&ok);
                //if (!ok) return false;
                 int randomNumber = QRandomGenerator::global()->bounded(val);
                value = QString::number(randomNumber);
                return true;
